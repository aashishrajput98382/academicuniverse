...
where $b=0$ denotes instances correct only in Pass A, and $c=99$ denotes instances correct only in Pass B, yielding $\chi^2 = 97.01$ ($p = 6.90 \times 10^{-23}$, highly significant). Furthermore, 10,000-iteration non-parametric bootstrap resampling in Table 12 establishes non-overlapping 95% confidence intervals between Pass A (F1: [76.89%, 82.22%]) and Pass B (F1: [88.56%, 92.44%]).
