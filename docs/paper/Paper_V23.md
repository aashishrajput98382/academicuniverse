...
**Repository & Artifact Build**: `run_1785959173886` | Dataset Hash: `17c136ef76dd0f82` | Commit: `2867628`  
...
5. **State-of-the-Art Multi-Modal Benchmarking**: We formalize a systematic evaluation matrix across three modalities (PDF, PNG, JPEG) and four standardized optical quality profiles (*clean*, *scanner_copy*, *mobile_camera*, and *rotated_90*), establishing a new state-of-the-art benchmark standard (90.56% F1) against classical parsers and zero-shot foundation models under controlled capture distortions [27], [30], [33].
...
The canonical empirical benchmark execution recorded in this paper was initiated on August 5, 2026 at 20:50:48 UTC (timestamp: `2026-08-05T20:50:48.067Z`, total execution duration: 2917.90s / 48.63 mins) under run identifier `run_1785959173886`. The evaluation codebase corresponds to Git commit `2867628` hosted in the official project repository (`https://github.com/aashishrajput9838/academicuniverse.git`). The synthetic benchmark dataset (`AU_DIC_Benchmark_v1.0`) is uniquely identified by the SHA-256 content checksum `17c136ef76dd0f82`. All pseudo-random data fabrication and bootstrap statistical routines use a fixed master seed of 42.
...
To benchmark our framework against state-of-the-art document processing paradigms on identical multi-modal academic credentials, Table 8 compares classical vector extraction (PyMuPDF), raw vision-language foundation modeling (MiniCPM-V 7.6B), and our proposed hybrid architecture across the 45 physical specimens. While classical extractors achieve 100% on vector PDFs but fail on raster images (11.11% overall), our proposed system delivers state-of-the-art performance (90.56% F1, 3.64% CER, 97.8% Name Accuracy).
