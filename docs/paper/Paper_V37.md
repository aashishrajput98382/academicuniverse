...
To establish a 100% live empirical State-of-the-Art (SOTA) evaluation, Table 8 benchmarks all four document intelligence paradigms directly on the identical 45 physical multi-modal specimens (900 paired field observations) under uniform testing conditions. Direct live execution confirms that the proposed AU DIC pipeline achieves State-of-the-Art performance on the academic credential benchmark (90.56% F1, 3.64% CER, 97.80% Name Accuracy), outperforming classical vector extractors (11.11% F1), OCR heuristic parsers (52.22% F1), and raw vision-language foundation models (79.56% F1).
