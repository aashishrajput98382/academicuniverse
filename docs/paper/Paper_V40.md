...
The evaluation benchmark suite (`AU_DIC_Benchmark_v1.0`) comprises 45 synthetic multi-modal academic credential specimens representing diverse physical and optical capture conditions evaluated across three core higher education document categories: Academic Certificates, Semester Marksheets, and Student Identity Cards. The benchmark dataset integrates three complementary document representations: Vector PDFs (5 specimens rendered directly from vector layout streams), Lossless PNG Scans (20 specimens across clean, flatbed scan, mobile capture, and 90° rotation), and Compressed JPEGs (20 specimens across clean, flatbed scan, mobile capture, and 90° rotation), totaling 900 evaluated ground-truth field observations (20 atomic fields per specimen). Table 3 details the multi-modal dataset composition, while Table 4 summarizes the physical and optical degradation parameters.
...
| **Total Multi-Modal Suite** | **5 Vector PDFs** | **20 Lossless PNGs** | **20 Compressed JPEGs** | **45 Synthetic Document Specimens (900 Fields)** |
...
The canonical empirical benchmark execution recorded in this paper was initiated on August 5, 2026 at 20:50:48 UTC (timestamp: `2026-08-05T20:50:48.067Z`, total execution duration: 2917.90s / 48.63 mins) under run identifier `run_1785959173886`. The evaluation codebase corresponds to Git commit `0bc63b0` hosted in the official project repository (`https://github.com/aashishrajput9838/academicuniverse.git`) [50]. The synthetic benchmark dataset (`AU_DIC_Benchmark_v1.0`) is uniquely identified by the SHA-256 content checksum `17c136ef76dd0f82`. All pseudo-random data fabrication and bootstrap statistical routines use a fixed master seed of 42.

---

## 5. Results & Discussion

The empirical evaluation of the proposed Smart Academic Document Intelligence System begins with dry-run infrastructure verification across all 45 synthetic document specimens in `AU_DIC_Benchmark_v1.0`. As detailed in Table 7, the framework validated zero database mutations, zero ground-truth leakage, and 100.00% verification accuracy, operating at a processing throughput of 242.59 specimens per second with 4.12 ms mean latency.
...
To establish a 100% live empirical State-of-the-Art (SOTA) evaluation, Table 8 benchmarks all four document intelligence paradigms directly on the identical 45 synthetic multi-modal document specimens (900 paired field observations) under uniform testing conditions. Direct live execution confirms that the proposed AU DIC pipeline achieves State-of-the-Art performance on the academic credential benchmark (90.56% F1, 3.64% CER, 97.80% Name Accuracy), outperforming classical vector extractors (11.11% F1), OCR heuristic parsers (52.22% F1), and raw vision-language foundation models (79.56% F1).
...
| Document Intelligence Architecture | Model Paradigm / Pipeline | Benchmark Evaluation Dataset | Category Accuracy | Student Name Accuracy | Overall Field F1 | Character Error Rate (CER) | Benchmark Status & Robustness Summary |
| :--- | :--- | :--- | :---: | :---: | :---: | :---: | :--- |
...
| **OCR + Spatial Heuristic Parser** [20] | Tesseract 5.3 + Layout Rules | **Direct Live Run (AU DIC 45 Specimens)** | 68.89% | 62.20% | **52.22%** | 38.40% | *Baseline: Degrades under optical noise & rotation* |
...
| **Proposed AU DIC System (Ours)** | **Neural VLM + 6-Stage CanonicalNormalizer** | **Direct Live Run (AU DIC 45 Specimens - Pass B)** | **100.00%** | **97.80%** | **90.56%** | **3.64%** | **🏆 State-of-the-Art (SOTA) on AU DIC Suite (+11.00% Gain)** |
