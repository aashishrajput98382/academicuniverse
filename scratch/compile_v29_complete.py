...
v29_pdf_path = workspace / "docs" / "paper" / "PaperV29_Ollama_Primary.pdf"
fig_dir = workspace / "docs" / "paper" / "extracted_figures"

doc = docx.Document(v28_docx_path)

# 1. Fix all 9 figures including new formal flowcharts
caption_map = {
    "Fig. 1.": "image1.png",
    "Fig. 2.": "image2.png",
    "Fig. 3.": "image3.png",
...
print("=== MAPPING ALL 9 FIGURES IN V29 WITH FORMAL FLOWCHARTS ===")
    ...
    for idx, p in enumerate(doc.paragraphs):
        ...
        if txt.startswith(prefix):
            ...
            for b in blips:
                ...
                print(f"[UPDATED] {prefix} -> {img_name}")
            break

# 2. Compact formatting for <= 20 pages
...
doc.save(v29_docx_path)
print(f"[SUCCESS] Saved PaperV29_Ollama_Primary.docx!")
...
try:
    doc_word = word.Documents.Open(os.path.abspath(str(v29_docx_path)))
    doc_word.SaveAs(os.path.abspath(str(v29_pdf_path)), FileFormat=17)
    page_count = doc_word.ComputeStatistics(2)
    doc_word.Close(False)
    print(f"[SUCCESS] Exported high-quality PDF: {v29_pdf_path.name} ({page_count} Pages!)")
finally:
    word.Quit()
