...
manifest_v13_path = workspace / "docs" / "paper" / "PAPER_V13_RELEASE_MANIFEST.md"
audit_v13_path = workspace / "docs" / "paper" / "PAPER_V13_CHANGE_AUDIT.md"

print("=================================================================")
print(" EXECUTING VERIFICATION & AUDIT FOR PAPER V13 REVISION (CONCLUSION & FUTURE WORK SWAPPED)")
print("=================================================================")
...
# 2. Verify V13 Artifacts Existence
assert v13_docx.exists(), "CRITICAL: PaperV13_Ollama_Primary.docx missing!"
assert v13_pdf.exists(), "CRITICAL: PaperV13_Ollama_Primary.pdf missing!"
assert v13_md.exists(), "CRITICAL: Paper_V13.md missing!"
print("[PASS] Paper V13 artifacts exist.")

# 3. Inspect V13 DOCX Sections 1-7 Structure
doc = docx.Document(v13_docx)
...
assert sec1_idx is not None, "Error: Section 1 heading not found in V13 DOCX body!"
assert sec2_idx is not None, "Error: Section 2 heading not found in V13 DOCX body!"
assert sec3_idx is not None, "Error: Section 3 heading ('3. Methodology') not found in V13 DOCX body!"
assert sec4_idx is not None, "Error: Section 4 heading not found in V13 DOCX body!"
assert sec5_idx is not None, "Error: Section 5 heading ('5. Results & Discussion') not found in V13 DOCX body!"
assert sec6_idx is not None, "Error: Section 6 heading ('6. Conclusion') not found in V13 DOCX body!"
assert sec7_idx is not None, "Error: Section 7 heading ('7. Future Work') not found in V13 DOCX body!"
assert ack_idx is not None, "Error: ACKNOWLEDGMENT heading not found in V13 DOCX body!"
assert ref_idx is not None, "Error: REFERENCES heading not found in V13 DOCX body!"
...
print(f"[INFO] Total tables in V13 DOCX: {total_tables}")
assert total_tables == 13, f"Error: Expected exactly 13 tables in V13 DOCX, found {total_tables}!"
...
print("[PASS] Table of Contents in V13 DOCX verified synchronized, clean, and renumbered (6. Conclusion, 7. Future Work).")

# Check PDF Page Count and Visual Integrity via pypdf
reader = pypdf.PdfReader(str(v13_pdf))
pdf_page_count = len(reader.pages)
print(f"[INFO] V13 PDF Page Count: {pdf_page_count} pages.")
assert pdf_page_count >= 20, f"Error: Unexpected page count {pdf_page_count}"

# Generate Release Manifest & Change Audit
sha_docx = hashlib.sha256(v13_docx.read_bytes()).hexdigest()
sha_pdf = hashlib.sha256(v13_pdf.read_bytes()).hexdigest()
sha_md = hashlib.sha256(v13_md.read_bytes()).hexdigest()

manifest_content = f"""# PAPER V13 RELEASE MANIFEST & SCIENTIFIC ARTIFACT AUDIT

**Release Version:** V13 (Conclusion & Future Work Swapped, Single ~100-Word Paragraphs)  
**Date of Release:** August 22, 2026  
**Venue Target:** IEEE Access / ICDAR 2026  
**Status:** **OFFICIAL V13 PRODUCTION BUILD** (V5 through V11 Frozen & Intact)
...
| :--- | :---: | :--- | :---: |
| `docs/paper/PaperV13_Ollama_Primary.docx` | {v13_docx.stat().st_size:,} | `{sha_docx}` | **VERIFIED** |
| `docs/paper/PaperV13_Ollama_Primary.pdf` | {v13_pdf.stat().st_size:,} | `{sha_pdf}` | **VERIFIED ({pdf_page_count} Pages)** |
| `docs/paper/Paper_V13.md` | {v13_md.stat().st_size:,} | `{sha_md}` | **VERIFIED** |
...
manifest_v13_path.write_text(manifest_content, encoding="utf-8")
print(f"[SUCCESS] Written {manifest_v13_path.name}")

audit_content = f"""# PAPER V13 CHANGE AUDIT REPORT: CONCLUSION & FUTURE WORK SWAP

**Date:** August 22, 2026  
**Auditor:** Automated Publication Pipeline & Verification Subsystem  
**Scope:** Paper V13 revision (`PaperV13_Ollama_Primary.docx`, `PaperV13_Ollama_Primary.pdf`, `Paper_V13.md`)
...
## 2. Structural Verification Matrix: V11 vs. V13

| Section / Element | Paper V11 | Paper V13 | Status |
| :--- | :--- | :--- | :---: |
...
audit_v13_path.write_text(audit_content, encoding="utf-8")
print(f"[SUCCESS] Written {audit_v13_path.name}")
print("=================================================================")
print(" ALL V13 VERIFICATION AND AUDIT CHECKS PASSED!")
print("=================================================================")
