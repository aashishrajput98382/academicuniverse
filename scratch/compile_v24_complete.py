...
# 1. Run pipeline
subprocess.run(["python", "scratch/generate_paperv24_pipeline.py"], check=True)

# 2. Fix exact figure mappings for all 9 figures
doc = docx.Document(v24_docx_path)
...
print("=== EXACT MAPPING OF ALL 9 FIGURES TO THEIR CAPTIONS IN V24 ===")
...
for tbl in doc.tables:
    for row in tbl.rows:
        for cell in row.cells:
            ...
            tcMar = parse_xml(f'<w:tcMar {nsdecls("w")}><w:top w:w="25" w:type="dxa"/><w:bottom w:w="25" w:type="dxa"/><w:left w:w="30" w:type="dxa"/><w:right w:w="30" w:type="dxa"/></w:tcMar>')
            tcPr.append(tcMar)
...
print(f"[SUCCESS] Saved compacted PaperV24_Ollama_Primary.docx!")
...
try:
    doc_word = word.Documents.Open(os.path.abspath(str(v24_docx_path)))
    doc_word.SaveAs(os.path.abspath(str(v24_pdf_path)), FileFormat=17)
