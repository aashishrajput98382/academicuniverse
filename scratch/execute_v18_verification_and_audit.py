...
manifest_v18_path = workspace / "docs" / "paper" / "PAPER_V18_RELEASE_MANIFEST.md"
audit_v18_path = workspace / "docs" / "paper" / "PAPER_V18_CHANGE_AUDIT.md"

print("=================================================================")
print(" EXECUTING VERIFICATION & AUDIT FOR PAPER V18 REVISION (CONCLUSION & FUTURE WORK SWAPPED)")
print("=================================================================")
...
# 2. Verify V18 Artifacts Existence
assert v18_docx.exists(), "CRITICAL: PaperV18_Ollama_Primary.docx missing!"
assert v18_pdf.exists(), "CRITICAL: PaperV18_Ollama_Primary.pdf missing!"
assert v18_md.exists(), "CRITICAL: Paper_V18.md missing!"
print("[PASS] Paper V18 artifacts exist.")

# 3. Inspect V18 DOCX Sections 1-7 Structure
doc = docx.Document(v18_docx)
...
assert sec1_idx is not None, "Error: Section 1 heading not found in V18 DOCX body!"
assert sec2_idx is not None, "Error: Section 2 heading not found in V18 DOCX body!"
assert sec3_idx is not None, "Error: Section 3 heading ('3. Methodology') not found in V18 DOCX body!"
assert sec4_idx is not None, "Error: Section 4 heading not found in V18 DOCX body!"
assert sec5_idx is not None, "Error: Section 5 heading ('5. Results & Discussion') not found in V18 DOCX body!"
assert sec6_idx is not None, "Error: Section 6 heading ('6. Conclusion') not found in V18 DOCX body!"
assert sec7_idx is not None, "Error: Section 7 heading ('7. Future Work') not found in V18 DOCX body!"
assert ack_idx is not None, "Error: ACKNOWLEDGMENT heading not found in V18 DOCX body!"
assert ref_idx is not None, "Error: REFERENCES heading not found in V18 DOCX body!"
...
print(f"[INFO] Total tables in V18 DOCX: {total_tables}")
assert total_tables == 13, f"Error: Expected exactly 13 tables in V18 DOCX, found {total_tables}!"
...
reader = pypdf.PdfReader(str(v18_pdf))
pdf_page_count = len(reader.pages)
assert 15 <= pdf_page_count <= 20, f"Error: Page count {pdf_page_count} violates <= 20 page limit!"
print(f"[PASS] Verified Paper V18 strictly satisfies journal page limitation: {pdf_page_count} pages (<= 20 Pages).")

# Generate Release Manifest & Change Audit
sha_docx = hashlib.sha256(v18_docx.read_bytes()).hexdigest()
sha_pdf = hashlib.sha256(v18_pdf.read_bytes()).hexdigest()
sha_md = hashlib.sha256(v18_md.read_bytes()).hexdigest()

manifest_content = f"""# PAPER V18 RELEASE MANIFEST & SCIENTIFIC ARTIFACT AUDIT

**Release Version:** V18 (Conclusion & Future Work Swapped, Single ~100-Word Paragraphs)  
**Date of Release:** August 22, 2026  
**Venue Target:** IEEE Access / ICDAR 2026  
**Status:** **OFFICIAL V18 PRODUCTION BUILD** (V5 through V11 Frozen & Intact)
...
| :--- | :---: | :--- | :---: |
| `docs/paper/PaperV18_Ollama_Primary.docx` | {v18_docx.stat().st_size:,} | `{sha_docx}` | **VERIFIED** |
| `docs/paper/PaperV18_Ollama_Primary.pdf` | {v18_pdf.stat().st_size:,} | `{sha_pdf}` | **VERIFIED ({pdf_page_count} Pages)** |
| `docs/paper/Paper_V18.md` | {v18_md.stat().st_size:,} | `{sha_md}` | **VERIFIED** |
...
manifest_v18_path.write_text(manifest_content, encoding="utf-8")
print(f"[SUCCESS] Written {manifest_v18_path.name}")

audit_content = f"""# PAPER V18 CHANGE AUDIT REPORT: CONCLUSION & FUTURE WORK SWAP

**Date:** August 22, 2026  
**Auditor:** Automated Publication Pipeline & Verification Subsystem  
**Scope:** Paper V18 revision (`PaperV18_Ollama_Primary.docx`, `PaperV18_Ollama_Primary.pdf`, `Paper_V18.md`)
...
## 2. Structural Verification Matrix: V11 vs. V18

| Section / Element | Paper V11 | Paper V18 | Status |
| :--- | :--- | :--- | :---: |
...
audit_v18_path.write_text(audit_content, encoding="utf-8")
print(f"[SUCCESS] Written {audit_v18_path.name}")
print("=================================================================")
print(" ALL V18 VERIFICATION AND AUDIT CHECKS PASSED!")
print("=================================================================")
