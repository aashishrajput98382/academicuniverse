...
manifest_v20_path = workspace / "docs" / "paper" / "PAPER_V20_RELEASE_MANIFEST.md"
audit_v20_path = workspace / "docs" / "paper" / "PAPER_V20_CHANGE_AUDIT.md"

print("=================================================================")
print(" EXECUTING VERIFICATION & AUDIT FOR PAPER V20 REVISION (CONCLUSION & FUTURE WORK SWAPPED)")
print("=================================================================")
...
# 2. Verify V20 Artifacts Existence
assert v20_docx.exists(), "CRITICAL: PaperV20_Ollama_Primary.docx missing!"
assert v20_pdf.exists(), "CRITICAL: PaperV20_Ollama_Primary.pdf missing!"
assert v20_md.exists(), "CRITICAL: Paper_V20.md missing!"
print("[PASS] Paper V20 artifacts exist.")

# 3. Inspect V20 DOCX Sections 1-7 Structure
doc = docx.Document(v20_docx)
...
assert sec1_idx is not None, "Error: Section 1 heading not found in V20 DOCX body!"
assert sec2_idx is not None, "Error: Section 2 heading not found in V20 DOCX body!"
assert sec3_idx is not None, "Error: Section 3 heading ('3. Methodology') not found in V20 DOCX body!"
assert sec4_idx is not None, "Error: Section 4 heading not found in V20 DOCX body!"
assert sec5_idx is not None, "Error: Section 5 heading ('5. Results & Discussion') not found in V20 DOCX body!"
assert sec6_idx is not None, "Error: Section 6 heading ('6. Conclusion') not found in V20 DOCX body!"
assert sec7_idx is not None, "Error: Section 7 heading ('7. Future Work') not found in V20 DOCX body!"
assert ack_idx is not None, "Error: ACKNOWLEDGMENT heading not found in V20 DOCX body!"
assert ref_idx is not None, "Error: REFERENCES heading not found in V20 DOCX body!"
...
print(f"[INFO] Total tables in V20 DOCX: {total_tables}")
assert total_tables == 13, f"Error: Expected exactly 13 tables in V20 DOCX, found {total_tables}!"
...
reader = pypdf.PdfReader(str(v20_pdf))
pdf_page_count = len(reader.pages)
assert 15 <= pdf_page_count <= 20, f"Error: Page count {pdf_page_count} violates <= 20 page limit!"
print(f"[PASS] Verified Paper V20 strictly satisfies journal page limitation: {pdf_page_count} pages (<= 20 Pages).")

# Generate Release Manifest & Change Audit
sha_docx = hashlib.sha256(v20_docx.read_bytes()).hexdigest()
sha_pdf = hashlib.sha256(v20_pdf.read_bytes()).hexdigest()
sha_md = hashlib.sha256(v20_md.read_bytes()).hexdigest()

manifest_content = f"""# PAPER V20 RELEASE MANIFEST & SCIENTIFIC ARTIFACT AUDIT

**Release Version:** V20 (Conclusion & Future Work Swapped, Single ~100-Word Paragraphs)  
**Date of Release:** August 22, 2026  
**Venue Target:** IEEE Access / ICDAR 2026  
**Status:** **OFFICIAL V20 PRODUCTION BUILD** (V5 through V11 Frozen & Intact)
...
| :--- | :---: | :--- | :---: |
| `docs/paper/PaperV20_Ollama_Primary.docx` | {v20_docx.stat().st_size:,} | `{sha_docx}` | **VERIFIED** |
| `docs/paper/PaperV20_Ollama_Primary.pdf` | {v20_pdf.stat().st_size:,} | `{sha_pdf}` | **VERIFIED ({pdf_page_count} Pages)** |
| `docs/paper/Paper_V20.md` | {v20_md.stat().st_size:,} | `{sha_md}` | **VERIFIED** |
...
manifest_v20_path.write_text(manifest_content, encoding="utf-8")
print(f"[SUCCESS] Written {manifest_v20_path.name}")

audit_content = f"""# PAPER V20 CHANGE AUDIT REPORT: CONCLUSION & FUTURE WORK SWAP

**Date:** August 22, 2026  
**Auditor:** Automated Publication Pipeline & Verification Subsystem  
**Scope:** Paper V20 revision (`PaperV20_Ollama_Primary.docx`, `PaperV20_Ollama_Primary.pdf`, `Paper_V20.md`)
...
## 2. Structural Verification Matrix: V11 vs. V20

| Section / Element | Paper V11 | Paper V20 | Status |
| :--- | :--- | :--- | :---: |
...
audit_v20_path.write_text(audit_content, encoding="utf-8")
print(f"[SUCCESS] Written {audit_v20_path.name}")
print("=================================================================")
print(" ALL V20 VERIFICATION AND AUDIT CHECKS PASSED!")
print("=================================================================")
