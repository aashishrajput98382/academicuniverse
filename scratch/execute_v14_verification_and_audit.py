...
manifest_v14_path = workspace / "docs" / "paper" / "PAPER_V14_RELEASE_MANIFEST.md"
audit_v14_path = workspace / "docs" / "paper" / "PAPER_V14_CHANGE_AUDIT.md"

print("=================================================================")
print(" EXECUTING VERIFICATION & AUDIT FOR PAPER V14 REVISION (CONCLUSION & FUTURE WORK SWAPPED)")
print("=================================================================")
...
# 2. Verify V14 Artifacts Existence
assert v14_docx.exists(), "CRITICAL: PaperV14_Ollama_Primary.docx missing!"
assert v14_pdf.exists(), "CRITICAL: PaperV14_Ollama_Primary.pdf missing!"
assert v14_md.exists(), "CRITICAL: Paper_V14.md missing!"
print("[PASS] Paper V14 artifacts exist.")

# 3. Inspect V14 DOCX Sections 1-7 Structure
doc = docx.Document(v14_docx)
...
assert sec1_idx is not None, "Error: Section 1 heading not found in V14 DOCX body!"
assert sec2_idx is not None, "Error: Section 2 heading not found in V14 DOCX body!"
assert sec3_idx is not None, "Error: Section 3 heading ('3. Methodology') not found in V14 DOCX body!"
assert sec4_idx is not None, "Error: Section 4 heading not found in V14 DOCX body!"
assert sec5_idx is not None, "Error: Section 5 heading ('5. Results & Discussion') not found in V14 DOCX body!"
assert sec6_idx is not None, "Error: Section 6 heading ('6. Conclusion') not found in V14 DOCX body!"
assert sec7_idx is not None, "Error: Section 7 heading ('7. Future Work') not found in V14 DOCX body!"
assert ack_idx is not None, "Error: ACKNOWLEDGMENT heading not found in V14 DOCX body!"
assert ref_idx is not None, "Error: REFERENCES heading not found in V14 DOCX body!"
...
print(f"[INFO] Total tables in V14 DOCX: {total_tables}")
assert total_tables == 13, f"Error: Expected exactly 13 tables in V14 DOCX, found {total_tables}!"
...
print("[PASS] Table of Contents in V14 DOCX verified synchronized, clean, and renumbered (6. Conclusion, 7. Future Work).")

# Check PDF Page Count and Visual Integrity via pypdf
reader = pypdf.PdfReader(str(v14_pdf))
pdf_page_count = len(reader.pages)
print(f"[INFO] V14 PDF Page Count: {pdf_page_count} pages.")
assert pdf_page_count >= 20, f"Error: Unexpected page count {pdf_page_count}"

# Generate Release Manifest & Change Audit
sha_docx = hashlib.sha256(v14_docx.read_bytes()).hexdigest()
sha_pdf = hashlib.sha256(v14_pdf.read_bytes()).hexdigest()
sha_md = hashlib.sha256(v14_md.read_bytes()).hexdigest()

manifest_content = f"""# PAPER V14 RELEASE MANIFEST & SCIENTIFIC ARTIFACT AUDIT

**Release Version:** V14 (Conclusion & Future Work Swapped, Single ~100-Word Paragraphs)  
**Date of Release:** August 22, 2026  
**Venue Target:** IEEE Access / ICDAR 2026  
**Status:** **OFFICIAL V14 PRODUCTION BUILD** (V5 through V11 Frozen & Intact)
...
| :--- | :---: | :--- | :---: |
| `docs/paper/PaperV14_Ollama_Primary.docx` | {v14_docx.stat().st_size:,} | `{sha_docx}` | **VERIFIED** |
| `docs/paper/PaperV14_Ollama_Primary.pdf` | {v14_pdf.stat().st_size:,} | `{sha_pdf}` | **VERIFIED ({pdf_page_count} Pages)** |
| `docs/paper/Paper_V14.md` | {v14_md.stat().st_size:,} | `{sha_md}` | **VERIFIED** |
...
manifest_v14_path.write_text(manifest_content, encoding="utf-8")
print(f"[SUCCESS] Written {manifest_v14_path.name}")

audit_content = f"""# PAPER V14 CHANGE AUDIT REPORT: CONCLUSION & FUTURE WORK SWAP

**Date:** August 22, 2026  
**Auditor:** Automated Publication Pipeline & Verification Subsystem  
**Scope:** Paper V14 revision (`PaperV14_Ollama_Primary.docx`, `PaperV14_Ollama_Primary.pdf`, `Paper_V14.md`)
...
## 2. Structural Verification Matrix: V11 vs. V14

| Section / Element | Paper V11 | Paper V14 | Status |
| :--- | :--- | :--- | :---: |
...
audit_v14_path.write_text(audit_content, encoding="utf-8")
print(f"[SUCCESS] Written {audit_v14_path.name}")
print("=================================================================")
print(" ALL V14 VERIFICATION AND AUDIT CHECKS PASSED!")
print("=================================================================")
