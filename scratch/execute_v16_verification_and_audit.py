...
manifest_v16_path = workspace / "docs" / "paper" / "PAPER_V16_RELEASE_MANIFEST.md"
audit_v16_path = workspace / "docs" / "paper" / "PAPER_V16_CHANGE_AUDIT.md"

print("=================================================================")
print(" EXECUTING VERIFICATION & AUDIT FOR PAPER V16 REVISION (CONCLUSION & FUTURE WORK SWAPPED)")
print("=================================================================")
...
# 2. Verify V16 Artifacts Existence
assert v16_docx.exists(), "CRITICAL: PaperV16_Ollama_Primary.docx missing!"
assert v16_pdf.exists(), "CRITICAL: PaperV16_Ollama_Primary.pdf missing!"
assert v16_md.exists(), "CRITICAL: Paper_V16.md missing!"
print("[PASS] Paper V16 artifacts exist.")

# 3. Inspect V16 DOCX Sections 1-7 Structure
doc = docx.Document(v16_docx)
...
assert sec1_idx is not None, "Error: Section 1 heading not found in V16 DOCX body!"
assert sec2_idx is not None, "Error: Section 2 heading not found in V16 DOCX body!"
assert sec3_idx is not None, "Error: Section 3 heading ('3. Methodology') not found in V16 DOCX body!"
assert sec4_idx is not None, "Error: Section 4 heading not found in V16 DOCX body!"
assert sec5_idx is not None, "Error: Section 5 heading ('5. Results & Discussion') not found in V16 DOCX body!"
assert sec6_idx is not None, "Error: Section 6 heading ('6. Conclusion') not found in V16 DOCX body!"
assert sec7_idx is not None, "Error: Section 7 heading ('7. Future Work') not found in V16 DOCX body!"
assert ack_idx is not None, "Error: ACKNOWLEDGMENT heading not found in V16 DOCX body!"
assert ref_idx is not None, "Error: REFERENCES heading not found in V16 DOCX body!"
...
print(f"[INFO] Total tables in V16 DOCX: {total_tables}")
assert total_tables == 13, f"Error: Expected exactly 13 tables in V16 DOCX, found {total_tables}!"
...
reader = pypdf.PdfReader(str(v16_pdf))
pdf_page_count = len(reader.pages)
assert 15 <= pdf_page_count <= 20, f"Error: Page count {pdf_page_count} violates <= 20 page limit!"
print(f"[PASS] Verified Paper V16 strictly satisfies journal page limitation: {pdf_page_count} pages (<= 20 Pages).")

# Generate Release Manifest & Change Audit
sha_docx = hashlib.sha256(v16_docx.read_bytes()).hexdigest()
sha_pdf = hashlib.sha256(v16_pdf.read_bytes()).hexdigest()
sha_md = hashlib.sha256(v16_md.read_bytes()).hexdigest()

manifest_content = f"""# PAPER V16 RELEASE MANIFEST & SCIENTIFIC ARTIFACT AUDIT

**Release Version:** V16 (Conclusion & Future Work Swapped, Single ~100-Word Paragraphs)  
**Date of Release:** August 22, 2026  
**Venue Target:** IEEE Access / ICDAR 2026  
**Status:** **OFFICIAL V16 PRODUCTION BUILD** (V5 through V11 Frozen & Intact)
...
| :--- | :---: | :--- | :---: |
| `docs/paper/PaperV16_Ollama_Primary.docx` | {v16_docx.stat().st_size:,} | `{sha_docx}` | **VERIFIED** |
| `docs/paper/PaperV16_Ollama_Primary.pdf` | {v16_pdf.stat().st_size:,} | `{sha_pdf}` | **VERIFIED ({pdf_page_count} Pages)** |
| `docs/paper/Paper_V16.md` | {v16_md.stat().st_size:,} | `{sha_md}` | **VERIFIED** |
...
manifest_v16_path.write_text(manifest_content, encoding="utf-8")
print(f"[SUCCESS] Written {manifest_v16_path.name}")

audit_content = f"""# PAPER V16 CHANGE AUDIT REPORT: CONCLUSION & FUTURE WORK SWAP

**Date:** August 22, 2026  
**Auditor:** Automated Publication Pipeline & Verification Subsystem  
**Scope:** Paper V16 revision (`PaperV16_Ollama_Primary.docx`, `PaperV16_Ollama_Primary.pdf`, `Paper_V16.md`)
...
## 2. Structural Verification Matrix: V11 vs. V16

| Section / Element | Paper V11 | Paper V16 | Status |
| :--- | :--- | :--- | :---: |
...
audit_v16_path.write_text(audit_content, encoding="utf-8")
print(f"[SUCCESS] Written {audit_v16_path.name}")
print("=================================================================")
print(" ALL V16 VERIFICATION AND AUDIT CHECKS PASSED!")
print("=================================================================")
