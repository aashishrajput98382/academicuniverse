...
manifest_v21_path = workspace / "docs" / "paper" / "PAPER_V21_RELEASE_MANIFEST.md"
audit_v21_path = workspace / "docs" / "paper" / "PAPER_V21_CHANGE_AUDIT.md"

print("=================================================================")
print(" EXECUTING VERIFICATION & AUDIT FOR PAPER V21 REVISION (CONCLUSION & FUTURE WORK SWAPPED)")
print("=================================================================")
...
assert v12_docx.exists(), "CRITICAL: Frozen PaperV12_Ollama_Primary.docx missing!"
...
assert v13_docx.exists(), "CRITICAL: Frozen PaperV13_Ollama_Primary.docx missing!"
...
assert v14_docx.exists(), "CRITICAL: Frozen PaperV14_Ollama_Primary.docx missing!"
...
assert v15_docx.exists(), "CRITICAL: Frozen PaperV15_Ollama_Primary.docx missing!"
...
assert v16_docx.exists(), "CRITICAL: Frozen PaperV16_Ollama_Primary.docx missing!"
...
assert v17_docx.exists(), "CRITICAL: Frozen PaperV17_Ollama_Primary.docx missing!"
...
assert v18_docx.exists(), "CRITICAL: Frozen PaperV18_Ollama_Primary.docx missing!"
...
assert v19_docx.exists(), "CRITICAL: Frozen PaperV19_Ollama_Primary.docx missing!"
...
assert v20_docx.exists(), "CRITICAL: Frozen PaperV20_Ollama_Primary.docx missing!"
...
# 2. Verify V21 Artifacts Existence
assert v21_docx.exists(), "CRITICAL: PaperV21_Ollama_Primary.docx missing!"
assert v21_pdf.exists(), "CRITICAL: PaperV21_Ollama_Primary.pdf missing!"
assert v21_md.exists(), "CRITICAL: Paper_V21.md missing!"
print("[PASS] Paper V21 artifacts exist.")

# 3. Inspect V21 DOCX Sections 1-7 Structure
doc = docx.Document(v21_docx)
...
for i, p in enumerate(doc.paragraphs):
    ...
    if txt == "1. Introduction":
        sec1_idx = i
    elif txt == "2. Related Work":
        sec2_idx = i
    elif txt == "3. Methodology":
        sec3_idx = i
    elif txt.startswith("4. Experimental Setup"):
        sec4_idx = i
    elif (txt == "5. Results & Discussion" or txt.startswith("5. Results") or txt.startswith("5. Result")) and sec4_idx is not None:
        sec5_idx = i
    elif (txt == "6. Conclusion" or txt.startswith("6. Conclusion")) and sec5_idx is not None:
        sec6_idx = i
    elif (txt == "7. Future Work" or txt.startswith("7. Future Work") or txt.startswith("7. Future Directions")) and sec6_idx is not None:
        sec7_idx = i
    elif txt.startswith("ACKNOWLEDGMENT") and sec7_idx is not None:
        ack_idx = i
    elif txt == "REFERENCES" and ack_idx is not None:
        ref_idx = i
        break

assert sec1_idx is not None, "Error: Section 1 heading not found in V21 DOCX body!"
assert sec2_idx is not None, "Error: Section 2 heading not found in V21 DOCX body!"
assert sec3_idx is not None, "Error: Section 3 heading ('3. Methodology') not found in V21 DOCX body!"
assert sec4_idx is not None, "Error: Section 4 heading not found in V21 DOCX body!"
assert sec5_idx is not None, "Error: Section 5 heading ('5. Results & Discussion') not found in V21 DOCX body!"
assert sec6_idx is not None, "Error: Section 6 heading ('6. Conclusion') not found in V21 DOCX body!"
assert sec7_idx is not None, "Error: Section 7 heading ('7. Future Work') not found in V21 DOCX body!"
assert ack_idx is not None, "Error: ACKNOWLEDGMENT heading not found in V21 DOCX body!"
assert ref_idx is not None, "Error: REFERENCES heading not found in V21 DOCX body!"
...
print(f"[INFO] Total tables in V21 DOCX: {total_tables}")
assert total_tables == 13, f"Error: Expected exactly 13 tables in V21 DOCX, found {total_tables}!"
...
# Verify standard research paper structure (no thesis front-matter, direct layout <= 20 pages)
print("[PASS] Standard research article structure verified: all 7 sections, 13 tables, 9 figures directly formatted.")

# Check PDF Page Count and Visual Integrity via pypdf
reader = pypdf.PdfReader(str(v21_pdf))
pdf_page_count = len(reader.pages)
assert 15 <= pdf_page_count <= 20, f"Error: Page count {pdf_page_count} violates <= 20 page limit!"
print(f"[PASS] Verified Paper V21 strictly satisfies journal page limitation: {pdf_page_count} pages (<= 20 Pages).")

# Generate Release Manifest & Change Audit
sha_docx = hashlib.sha256(v21_docx.read_bytes()).hexdigest()
sha_pdf = hashlib.sha256(v21_pdf.read_bytes()).hexdigest()
sha_md = hashlib.sha256(v21_md.read_bytes()).hexdigest()

manifest_content = f"""# PAPER V21 RELEASE MANIFEST & SCIENTIFIC ARTIFACT AUDIT

**Release Version:** V21 (Conclusion & Future Work Swapped, Single ~100-Word Paragraphs)  
**Date of Release:** August 22, 2026  
**Venue Target:** IEEE Access / ICDAR 2026  
**Status:** **OFFICIAL V21 PRODUCTION BUILD** (V5 through V11 Frozen & Intact)
...
| :--- | :---: | :--- | :---: |
| `docs/paper/PaperV21_Ollama_Primary.docx` | {v21_docx.stat().st_size:,} | `{sha_docx}` | **VERIFIED** |
| `docs/paper/PaperV21_Ollama_Primary.pdf` | {v21_pdf.stat().st_size:,} | `{sha_pdf}` | **VERIFIED ({pdf_page_count} Pages)** |
| `docs/paper/Paper_V21.md` | {v21_md.stat().st_size:,} | `{sha_md}` | **VERIFIED** |
...
manifest_v21_path.write_text(manifest_content, encoding="utf-8")
print(f"[SUCCESS] Written {manifest_v21_path.name}")

audit_content = f"""# PAPER V21 CHANGE AUDIT REPORT: CONCLUSION & FUTURE WORK SWAP

**Date:** August 22, 2026  
**Auditor:** Automated Publication Pipeline & Verification Subsystem  
**Scope:** Paper V21 revision (`PaperV21_Ollama_Primary.docx`, `PaperV21_Ollama_Primary.pdf`, `Paper_V21.md`)
...
## 2. Structural Verification Matrix: V11 vs. V21

| Section / Element | Paper V11 | Paper V21 | Status |
| :--- | :--- | :--- | :---: |
...
audit_v21_path.write_text(audit_content, encoding="utf-8")
print(f"[SUCCESS] Written {audit_v21_path.name}")
print("=================================================================")
print(" ALL V21 VERIFICATION AND AUDIT CHECKS PASSED!")
print("=================================================================")
