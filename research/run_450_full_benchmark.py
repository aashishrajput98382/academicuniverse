#!/usr/bin/env python3
"""
research/run_450_full_benchmark.py

Full-Scale 450-Document Scientific Benchmark Evaluation Pipeline.
Strictly executes the validated AU DIC evaluation framework against:
- 50 Clean PDFs
- 50 Clean PNGs
- 50 Scanned PNGs (scanner_copy)
- 50 Mobile Taken PNGs (mobile_camera)
- 50 Rotated PNGs (rotated_90)
- 50 Clean JPEGs
- 50 Scanned JPEGs (scanner_copy)
- 50 Mobile Taken JPEGs (mobile_camera)
- 50 Rotated JPEGs (rotated_90)
Total: Exactly 450 Specimens (9,000 Atomic Field Observations).
...
def collect_450_specimens():
    """
    Indexes exactly 450 specimens (50 per each of 9 categories)
...
def run_benchmark():
    run_id = f"run_450_benchmark_{int(time.time() * 1000)}"
    ...
    log("=================================================================")
    log(" AU DIC 450-DOCUMENT FULL BENCHMARK EVALUATION ENGINE")
    log(f" Run ID: {run_id}")
    log(f" Output Directory: {run_dir}")
    log("=================================================================")
    
    # 1. Collect and verify 450 specimens
    log("[STEP 1/5] Collecting and verifying 450 multi-modal specimens...")
    specimens = collect_450_specimens()
    total_docs = len(specimens)
    log(f"[PASS] Successfully verified exactly {total_docs} specimens across 9 categories (50 each).")
    ...
    for idx, spec in enumerate(specimens, 1):
        ...
        try:
            ...
            for f_type, gt_val in fields_to_eval:
                ...
                # Model extraction logic identical to validated protocol
        ...
        # Periodic Progress & Checkpointing (every 50 documents)
        if idx % 50 == 0 or idx == total_docs:
            ...
            log(f"  [PROGRESS {pct:5.1f}%] Completed: {idx:3d}/{total_docs:3d} | Success: {successful_docs:3d} | Failed: {failed_docs} | Rate: {rate:.1f} docs/sec | ETA: {eta_str}")
    ...
    log(f"Total Paired Field Observations: {N_total:,} rows (450 documents * 20 fields)")
    ...
    # 6. Summary Markdown
    summary_md = f"""# AU DIC 450-Document Scientific Benchmark Report

**Run ID**: `{run_id}`  
**Dataset Source**: `{DATASET_DIR}`  
**Total Evaluated Documents**: **{total_docs}** ({successful_docs} Successful | {failed_docs} Failed)  
**Total Paired Field Observations**: **{N_total:,}** (450 specimens $\\times$ 20 fields)  
...
| :--- | :--- | :--- | :--- | :--- |
| **Clean Vector PDF** | 50 | {comparisons_payload[0]['pass_a_f1']*100:.2f}% | {comparisons_payload[0]['pass_b_f1']*100:.2f}% | +{(comparisons_payload[0]['pass_b_f1']-comparisons_payload[0]['pass_a_f1'])*100:.2f}% |
| **Clean PNG** | 50 | {comparisons_payload[1]['pass_a_f1']*100:.2f}% | {comparisons_payload[1]['pass_b_f1']*100:.2f}% | +{(comparisons_payload[1]['pass_b_f1']-comparisons_payload[1]['pass_a_f1'])*100:.2f}% |
| **Scanned PNG (scanner_copy)** | 50 | {comparisons_payload[2]['pass_a_f1']*100:.2f}% | {comparisons_payload[2]['pass_b_f1']*100:.2f}% | +{(comparisons_payload[2]['pass_b_f1']-comparisons_payload[2]['pass_a_f1'])*100:.2f}% |
| **Mobile Camera PNG** | 50 | {comparisons_payload[3]['pass_a_f1']*100:.2f}% | {comparisons_payload[3]['pass_b_f1']*100:.2f}% | +{(comparisons_payload[3]['pass_b_f1']-comparisons_payload[3]['pass_a_f1'])*100:.2f}% |
| **Rotated 90° PNG** | 50 | {comparisons_payload[4]['pass_a_f1']*100:.2f}% | {comparisons_payload[4]['pass_b_f1']*100:.2f}% | +{(comparisons_payload[4]['pass_b_f1']-comparisons_payload[4]['pass_a_f1'])*100:.2f}% |
| **Clean JPEG** | 50 | {comparisons_payload[5]['pass_a_f1']*100:.2f}% | {comparisons_payload[5]['pass_b_f1']*100:.2f}% | +{(comparisons_payload[5]['pass_b_f1']-comparisons_payload[5]['pass_a_f1'])*100:.2f}% |
| **Scanned JPEG (scanner_copy)** | 50 | {comparisons_payload[6]['pass_a_f1']*100:.2f}% | {comparisons_payload[6]['pass_b_f1']*100:.2f}% | +{(comparisons_payload[6]['pass_b_f1']-comparisons_payload[6]['pass_a_f1'])*100:.2f}% |
| **Mobile Camera JPEG** | 50 | {comparisons_payload[7]['pass_a_f1']*100:.2f}% | {comparisons_payload[7]['pass_b_f1']*100:.2f}% | +{(comparisons_payload[7]['pass_b_f1']-comparisons_payload[7]['pass_a_f1'])*100:.2f}% |
| **Rotated 90° JPEG** | 50 | {comparisons_payload[8]['pass_a_f1']*100:.2f}% | {comparisons_payload[8]['pass_b_f1']*100:.2f}% | +{(comparisons_payload[8]['pass_b_f1']-comparisons_payload[8]['pass_a_f1'])*100:.2f}% |
| **OVERALL SYSTEM TOTAL** | **450** | **{f1_a*100:.2f}%** | **{f1_b*100:.2f}%** | **+{delta_f1*100:.2f}% ({rel_f1:+.2f}%)** |
...
"""
    ...
    log("\n=================================================================")
    log(" 450-DOCUMENT BENCHMARK RUN COMPLETED SUCCESSFULLY!")
